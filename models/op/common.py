...
def load_pretrained_model(model, path, strict=False):
...
def unnormalize(t):
    t = (t + 1) * 127.5
    return torch.clamp(t, 0, 255).to(torch.uint8)
...
# copy from https://github.com/Janspiry/Image-Super-Resolution-via-Iterative-Refinement/blob/master/model/ddpm_modules/diffusion.py
import numpy as np
import math
import torch
...
def make_beta_schedule(schedule, n_timestep, linear_start=1e-4, linear_end=2e-2, cosine_s=8e-3):
    ...
    # if betas is torch.Tensor, return 
...
def extract(a, t, x_shape):
    b, *_ = t.shape
    # check device
    assert t.device == a.device, 't and a must be on the same device'
    out = a.gather(-1, t)
    return out.reshape(b, *((1,) * (len(x_shape) - 1)))

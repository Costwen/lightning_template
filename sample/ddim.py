...
from samples.utils import extract as extract_into_tensor
from scripts.utils import unnormalize
...
class DDIM():
    def __init__(self, model, ddim_num_timesteps, schedule="linear", eta=0., verbose=True,**kwargs):
        super().__init__()
        self.model = model
        ...
        self.eta = eta
        print(f'Data shape for DDIM sampling eta {eta}')
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_num_steps=self.ddim_num_timesteps, ddim_eta=self.eta, verbose=self.verbose)
        if "shape" in kwargs:
            kwargs.pop("shape")
    ...
    def ddim_sampling(self, cond, shape,
        ...
        return {"samples": unnormalize(img), "intermediates": intermediates}
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            e_t = self.model.denoise_fn(x, t,lower_x=lower_x, cond=c)
        else:
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
            c_in = torch.cat([unconditional_conditioning, c])
            e_t_uncond, e_t = self.model.denoise_fn(x_in, t_in, cond=c_in).chunk(2)
